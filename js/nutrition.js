...
export function targets(profile, weightKg) {
  ...
  const floor = profile.sex === 'F' ? 1100 : 1300;
  let kcal = profile.goalKcal || Math.round(tdee - (profile.deficit || 500));
  kcal = Math.max(floor, Math.round(kcal / 10) * 10);
  const prot = Math.round(w * (profile.proteinPerKg || 1.4));
  return {
    kcal, prot, bmr: Math.round(b), tdee: Math.round(tdee),
    carb: Math.round((kcal * 0.40) / 4), fat: Math.round((kcal * 0.28) / 9),
